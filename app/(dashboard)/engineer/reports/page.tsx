...
export default function EngineerReportsPage() {
  ...
  // モックデータから稼働報告を取得
  const reportsByDate = mockWorkReports.reduce(
    (acc, report) => {
    ...
    <div className="container py-6 space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground">日々の稼働時間と作業内容を記録</p>
        </div>
                        ...
                        {hasReport && (
                          <div
                            className={`absolute bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full ${
                              hasReport.status === 'approved'
                                ? 'bg-green-500'
                                : hasReport.status === 'submitted'
                                  ? 'bg-blue-500'
                                  : hasReport.status === 'rejected'
                                    ? 'bg-red-500'
                                    : 'bg-yellow-500'
                            }`}
                          />
            ...
            {reportMode === 'daily' ? (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>稼働時間と業務内容を記録</CardDescription>
                </CardHeader>
                <CardContent>
                  {/* 打刻エリア */}
                  <Card className="mb-6">
