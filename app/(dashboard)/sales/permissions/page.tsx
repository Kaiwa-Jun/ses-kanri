...
import { 
  Shield, Users, Building2, Search, Save, Plus, Edit2, History,
...
export default function PermissionsPage() {
  ...
  const getPermissionText = (level: string) => {
    switch (level) {
      ...
      case "own": return "担当のみ";
      case "none": return "非表示";
  ...
  const getPermissionIcon = (level: string) => {
    switch (level) {
      case "all": return <CheckCircle2 className="h-4 w-4" />;
      case "team": return <Users className="h-4 w-4" />;
      case "own": return <AlertCircle className="h-4 w-4" />;
                ...
                <TableBody>
                  {permissions.individuals.map(member => (
                    <TableRow key={member.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={getPermissionColor(member.permissions.engineers)}>
                          {getPermissionIcon(member.permissions.engineers)}
                          {getPermissionText(member.permissions.engineers)}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={getPermissionColor(member.permissions.projects)}>
                          {getPermissionIcon(member.permissions.projects)}
                          {getPermissionText(member.permissions.projects)}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={getPermissionColor(member.permissions.clients)}>
                          {getPermissionIcon(member.permissions.clients)}
                          {getPermissionText(member.permissions.clients)}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={getPermissionColor(member.permissions.revenue)}>
                      ...
                      <TableCell>
                        <Badge variant="secondary">
                          {permissionTemplates.find(t => t.id === member.template)?.name || "カスタム"}
                        </Badge>
                      </TableCell>
                  ...
                  <Card key={template.id}>
                    ...
                    <CardContent className="space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <Badge variant="outline" className={getPermissionColor(template.permissions.engineers)}>
                          {getPermissionText(template.permissions.engineers)}
                        </Badge>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm">案件情報</span>
                        <Badge variant="outline" className={getPermissionColor(template.permissions.projects)}>
                          {getPermissionText(template.permissions.projects)}
                        </Badge>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm">企業情報</span>
                        <Badge variant="outline" className={getPermissionColor(template.permissions.clients)}>
                          {getPermissionText(template.permissions.clients)}
                        </Badge>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm">売上情報</span>
                        <Badge variant="outline" className={getPermissionColor(template.permissions.revenue)}>
                ...
                <TableBody>
                  {permissions.teams.map(team => (
                    <TableRow key={team.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={getPermissionColor(team.permissions.engineers)}>
                          {getPermissionIcon(team.permissions.engineers)}
                          {getPermissionText(team.permissions.engineers)}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={getPermissionColor(team.permissions.projects)}>
                          {getPermissionIcon(team.permissions.projects)}
                          {getPermissionText(team.permissions.projects)}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={getPermissionColor(team.permissions.clients)}>
                          {getPermissionIcon(team.permissions.clients)}
                          {getPermissionText(team.permissions.clients)}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={getPermissionColor(team.permissions.revenue)}>
