...
import { FreezeConfirmationModal } from '../freeze-confirmation-modal';
...
describe('FreezeConfirmationModal', () => {
  const defaultProps = {
    open: true,
  ...
  it('単一凍結モーダルが正しくレンダリングされる', () => {
    render(<FreezeConfirmationModal {...defaultProps} />);

    // タイトルの確認
    expect(screen.getByText('凍結の確認')).toBeInTheDocument();

    // メッセージの確認
    expect(screen.getByText('選択した加盟店を凍結します。')).toBeInTheDocument();
  ...
  it('複数凍結モーダルが正しくレンダリングされる', () => {
    ...
    // メッセージの確認
    expect(screen.getByText('選択した2件の加盟店を凍結します。')).toBeInTheDocument();
    expect(screen.getByText('凍結対象の加盟店：')).toBeInTheDocument();
  ...
  it('モーダルが閉じている時は表示されない', () => {
    render(<FreezeConfirmationModal {...defaultProps} open={false} />);

    expect(screen.queryByText('凍結の確認')).not.toBeInTheDocument();
  });
  ...
  it('キャンセルボタンをクリックするとonOpenChangeが呼ばれる', async () => {
    ...
    render(<FreezeConfirmationModal {...defaultProps} onOpenChange={onOpenChange} />);
  ...
  it('プロパティが正しく適用される', () => {
    ...
    render(<FreezeConfirmationModal {...customProps} />);

    expect(screen.getByText('加盟店名：カスタムストア名')).toBeInTheDocument();
  });

  it('selectedStoresが空の場合は単一凍結として動作する', () => {
    render(
      <FreezeConfirmationModal
        {...defaultProps}
        isMultiple={false}
    ...
    // リストは表示されない
    expect(screen.queryByText('凍結対象の加盟店：')).not.toBeInTheDocument();
  });
