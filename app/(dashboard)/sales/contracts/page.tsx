...
import { 
  FileText, Search, Filter, Calendar, Clock, AlertTriangle, CheckCircle, 
  XCircle, PlayCircle, PauseCircle, MoreHorizontal, Plus, User, Building2
} from "lucide-react";
...
export default function ContractsPage() {
  ...
  const actionRequiredContracts = contracts.filter(contract => 
    contract.hasNotification && contract.actionStatus !== "completed"
  );
  
  const noActionRequiredContracts = contracts.filter(contract => 
    !contract.hasNotification || contract.actionStatus === "completed"
  );
  ...
  const updateContractStatus = (contractId: string, newStatus: string) => {
    setContracts(prev => prev.map(c => 
      c.id === contractId ? { ...c, actionStatus: newStatus } : c
    ));
  };
  ...
  const getContractStatusColor = (status: string) => {
    switch (status) {
      ...
      case "expiring": return "text-red-500 bg-red-100 dark:bg-red-900/30";
          ...
          <div 
            className={`p-3 rounded-lg border transition-colors hover:bg-muted/50 cursor-pointer ${
              contract.hasNotification && showActionStatus ? 
                contract.notificationPriority === "high" ? "bg-red-50 dark:bg-red-900/10 border-red-200" : "bg-yellow-50 dark:bg-yellow-900/10 border-yellow-200"
                : "border-border"
            }`}
            ...
            <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {contract.hasNotification && showActionStatus && (
                    <AlertTriangle className={`h-4 w-4 flex-shrink-0 ${
                      contract.notificationPriority === "high" ? "text-red-500" : "text-yellow-500"
                    }`} />
                  )}
                </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {contract.status !== "active" && (
                  <Badge variant="outline" className={`${getContractStatusColor(contract.status)} text-xs px-2 py-0`}>
                ...
                {showActionStatus && (
                  <Badge variant="outline" className={`${statusInfo.color} text-xs px-2 py-0 gap-1`}>
                    {statusInfo.icon}
                    {statusInfo.text}
                  </Badge>
                )}
              ...
              <div className="flex items-center gap-4 flex-shrink-0">
                ...
                <div className="text-xs text-center">
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className={`${daysUntilDue <= 30 && showActionStatus ? "text-red-600 font-medium" : "text-muted-foreground"}`}>
                      {daysUntilDue}日後
                    </span>
                  </div>
      ...
      {filteredActionRequired.length === 0 && filteredNoActionRequired.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted-foreground mb-4">
            {searchTerm || filterStatus !== "all" ? "検索条件に一致する契約がありません" : "契約が登録されていません"}
          </p>
              ...
              <div className="p-4 bg-muted/50 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className={`${getDaysUntilDue(selectedContract.dueDate) <= 7 ? "text-red-600 font-medium" : ""}`}>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className={`text-sm ${
                            getDaysUntilDue(item.dueDate) <= 3 ? "text-red-600 font-medium" : "text-muted-foreground"
                          }`}>
